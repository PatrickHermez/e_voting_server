...
// endpoint to return all values decrypted from the database
app.get(routes.get_decrypted_values, async (req, res) => {
  const voters = await prisma.voter.findMany();
  const lists = voters.map((voter) => voter.list);
  const candidates = voters.map((voter) => voter.candidate);

  const decrypted_voters = await getPythonProcess("pylib/addition.py", [
    lists,
    candidates,
  ]);

  res.status(200).json(decrypted_voters);
});
